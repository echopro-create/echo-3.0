...
import ErrorFace from "@/app/components/ErrorFace";
...
export default function PublicShareClient({ token }: { token: string }) {
  ...
  return (
    <main className="px-4 py-6" style={{ maxWidth: 980, margin: "0 auto" }}>
      ...
      {/* Лица ошибок (кроме 401) */}
      {!needsPassword(error) && error && (
        <>
          {error === "not_found" ? (
            <ErrorFace variant={404} className="mb-4" />
          ) : error === "gone" ? (
            <ErrorFace variant={410} className="mb-4" />
          ) : (
            <ErrorFace variant={"error"} className="mb-4" />
          )}
        </>
      )}
          ...
          <section className="card p-4 grid gap-2 mb-4">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-sm opacity-70">Истекает: {safeDate(share?.expires_at ?? null)}</span>
                <span className="text-sm opacity-70">Пароль: {share?.password_protected ? "включён" : "выключен"}</span>
                {share?.last_view_at ? (
...
function BadgeStatus({ code, label }: { code: "active" | "expired" | "revoked" | "overlimit" | "unknown"; label: string }) {
