...
export default async function MessagesPage() {
  ...
  // Благодаря RLS получаем только свои записи
    ...
    <div className="container py-6 grid gap-4">
      ...
      <div className="grid gap-3">
        {items.length === 0 ? (
          ...
          items.map(m => (
            <a key={m.id} className="card block hover:shadow-sm transition-shadow" href={`/s/${m.id}`}>
              ...
              {m.content && m.kind === 'text' ? (
                ...
                <div className="mt-2 text-[var(--mute)]">Без предпросмотра</div>
              )}
