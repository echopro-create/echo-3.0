...
export default function LoginPage() {
  ...
  const verifyOtp = useCallback(async () => {
    ...
    setLoading(true);
    try {
      // Верифицируем код через Supabase (клиент)
      ...
      const expiresIn = data.session?.expires_in ?? 60 * 60; // сек

      if (!accessToken) throw new Error('Нет access token в ответе');

      // Ставим httpOnly куку на сервере, чтобы middleware пропускал
      ...
      // Готово: ведём пользователя по next
      router.replace(next);
    } catch (e: any) {
